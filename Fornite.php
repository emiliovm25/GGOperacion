<?php
$api_key = "4cdf25a7-12d3-47aa-837a-b940bf9884c2";
$url = "https://fortnite-api.com/v2/news?language=es";
...
// Verificar y extraer noticias
if ($data && isset($data['status'])) {
    if ($data['status'] == 200 && isset($data['data']['br']['motds'])) {
        $noticias = $data['data']['br']['motds'];
    }
}
?>
...
/* Nuevo contenedor para el texto y el video */
.modes-header {
    display: flex;
    align-items: center;
    justify-content: center;
...
/* Estilos para el texto de "Múltiples maneras de jugar" */
.modes-text {
    text-align: right;
    color: white;
...
<body>
    ...
    <div class="fortnite-hero">
        ...
        <div class="container hero-content">
            ...
            <a href="https://launcher-public-service-prod06.ol.epicgames.com/launcher/api/installer/download/EpicGamesLauncherInstaller.msi?platformName=fortnite" target="_blank" rel="noopener noreferrer" class="fortnite-btn-link">
                <button class="fortnite-btn">JUEGA YA</button>
            </a>
        </div>
        
    </div>
    <h1 class="news-title" style="text-align: center; width: 100%; margin: 40px 0; font-size: 4rem; font-weight: 900; text-transform: uppercase; color:rgb(212, 175, 55, 0.5); letter-spacing: 2px;">NOTICIAS</h1>
    <?php
    $counter = 0;
    foreach ($noticias as $noticia):
    ?>
        <div class="fortnite-news-card">
            <div class="news-image">
                <img src="<?= htmlspecialchars($noticia['image'] ?? '') ?>" alt="<?= htmlspecialchars($noticia['title'] ?? '') ?>">
            </div>
            <div class="news-content">
                <h3 class="news-title"><?= htmlspecialchars($noticia['title'] ?? 'Sin título') ?></h3>
                <p class="news-body"><?= htmlspecialchars($noticia['body'] ?? '') ?></p>
            </div>
        </div>
    <?php endforeach; ?>
    </div>
  <div class="valorant-container">
    <div class="valorant-card">
        <div class="valorant-image">
            <img src="imglogo/modosfor.png" alt="Modos de Fortnite">
        </div>
        <div class="valorant-content">
            <h2 class="valorant-title">MODOS DE JUEGO</h2>
            <p class="valorant-body">
                DIVERSIÓN PARA TODOS.<br><br>
                Disfruta del Battle Royale clásico, el modo Creativo o los modos temporales. Cada opción ofrece una experiencia única con mecánicas diferentes para que nunca te aburras.
            </p>
             <a href="fornitecontenido.php?seccion=modo" class="valorant-btn">VER Modos</a>
        </div>
    </div>
</div>
<div class="valorant-container">
    <div class="valorant-card">
        <div class="valorant-image">
            <img src="imglogo/tiendafor.png" alt="Tienda de Fortnite">
        </div>
        <div class="valorant-content">
            <h2 class="valorant-title">TIENDA</h2>
            <p class="valorant-body">
                NUEVOS ITEMS CADA DÍA.<br><br>
                Visita la tienda para descubrir skins, picos, emotes y paquetes especiales. La selección cambia regularmente con contenido nuevo para personalizar tu personaje.
            </p>
            <a href="fornitecontenido.php?seccion=tienda" class="valorant-btn">IR ALA TIENDA</a>
        </div>
    </div>
</div>
<div class="valorant-container">
    <div class="valorant-card">
        <div class="valorant-image">
            <img src="imglogo/cosmetifor.png" alt="Cosméticos de Fortnite">
        </div>
        <div class="valorant-content">
            <h2 class="valorant-title">COSMÉTICOS</h2>
            <p class="valorant-body">
                PERSONALIZA TU ESTILO.<br><br>
                Elige entre cientos de skins, mochilas, gestos y armas decoradas. Mezcla y combina para crear un look único que te represente en el campo de batalla.
            </p>
             <a href="fornitecontenido.php?seccion=cosmeticos" class="valorant-btn">VER COSMÉTICOS</a>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Elementos del DOM
        const modeButtons = document.querySelectorAll('.compact-mode-btn');
        const video = document.getElementById('modesVideo');
        const loadingIndicator = document.querySelector('.video-loading');
        const modeContents = document.querySelectorAll('.compact-mode-content');
        ...
        video.addEventListener('error', function() {
            hideLoading();
            console.error("Error al cargar el video:", this.src);
            // Puedes mostrar un mensaje o imagen de respaldo aquí
        });

        video.addEventListener('loadeddata', function() {
            hideLoading();
            // Forzar reproducción (algunos navegadores bloquean autoplay sin interacción)
            video.play().catch(e => {
                console.log("Autoplay bloqueado, mutando video...");
                video.muted = true;
        ...
        modeButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Cambiar video solo si es diferente al actual
                if (video.src !== videoSrc) {
                    video.src = videoSrc;
                    // El evento loadeddata manejará el hideLoading()
                } else {
                    hideLoading();
                }
            });
        ...
        function preloadVideos() {
            const videoSources = Array.from(modeButtons).map(btn => btn.dataset.video);
